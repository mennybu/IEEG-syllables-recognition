...
class MHA_block(nn.Module):
    def __init__(self, d_core,seq_length = 3000 , d_embed=None, dropout=0, heads=4):
...
class TransformerWithSepChannels1DConv(nn.Module):
    def __init__(self,d_core, n_classes, seq_length, kernel_size, d_embed=None, dropout=0, heads=4, conv_in_channels=126, conv_out_channels=5):
        ...
        self.conv1D = nn.Conv1d(in_channels=conv_in_channels, out_channels=conv_out_channels, kernel_size=kernel_size, stride=kernel_size )
        l_out = int(seq_length / kernel_size)
        # self.fully_connected = nn.Linear(self.d_embed * conv_out_channels *  l_out, n_classes)
        self.fully_connected = nn.Linear(conv_out_channels *  l_out, n_classes)
    def forward(self, x):
        x = self.MHA(x)
        print(x.size())
        x = x.permute(0, 2, 1) #for conv1D not sep
        x = self.conv1D(x)
        print(x.size())
...
if __name__ == "__main__":
    ...
    model_dir = f'/home/tauproj6/EEG_proj/patient_1_output_1.5s/annotate_model_25.07_transformer_conv1D_not_sep/'
