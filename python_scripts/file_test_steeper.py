...
def preprocess_and_filter_data(data, sampling_rate=2000):
    ...
    print(f"After CAR range: {np.min(car_data)} to {np.max(car_data)}")
    
    # Step 2: Notch filters at 50Hz and 100Hz (steeper)
    notch_freq = [50, 100]
    ...
    print(f"After notch filter range: {np.min(car_data)} to {np.max(car_data)}")
    
    # Step 3: Define filter parameters for specific bands
    lowpass_high = 100 / nyquist_frequency
    highpass_low = 70 / nyquist_frequency
    highpass_high = 150 / nyquist_frequency
    
    # Step 4: Design and apply the specific band filters (steeper)
    b_low, a_low = signal.ellip(8, 0.1, 80, lowpass_high, btype='low')
    b_high, a_high = signal.ellip(8, 0.1, 80, [highpass_low, highpass_high], btype='band')
    
    low_filtered = signal.filtfilt(b_low, a_low, car_data, axis=1)
    high_filtered = signal.filtfilt(b_high, a_high, car_data, axis=1)
    
    print(f"After band filters - Low range: {np.min(low_filtered)} to {np.max(low_filtered)}")
    print(f"After band filters - High range: {np.min(high_filtered)} to {np.max(high_filtered)}")
    
    # Step 5: Convert high-frequency data to amplitude envelope
    #analytic_signal = hilbert(high_filtered, axis=1)
    #amplitude_envelope = np.abs(analytic_signal)
    
   # print(f"Amplitude envelope range: {np.min(amplitude_envelope)} to {np.max(amplitude_envelope)}")
    
    # Step 6: Apply sliding-window z-score normalization
    low_freq_zscore = np.apply_along_axis(sliding_window_zscore, 1, low_filtered)
    high_freq_zscore = np.apply_along_axis(sliding_window_zscore, 1, high_filtered)
    
    print(f"Z-score ranges - Low: {np.min(low_freq_zscore)} to {np.max(low_freq_zscore)}")
    print(f"Z-score ranges - High: {np.min(high_freq_zscore)} to {np.max(high_freq_zscore)}")
    
    # Step 7: Create 2D array with z-scored low-frequency data and z-scored amplitude envelope of high-frequency data
    filtered_array = np.vstack((low_freq_zscore, high_freq_zscore))
    
    print(f"Final output shape: {filtered_array.shape}")
    print(f"Final output range: {np.min(filtered_array)} to {np.max(filtered_array)}")
...
def main():
    ...
    for event in event_data:
        ...
        output_dir = rf"/home/tauproj6/EEG_proj/patient_1_1s_preproccesed_steeper/{label_to_info[label][1]}/{label_to_info[label][0]}"
