...
class SeparateConv1DClassifier(nn.Module):
    def __init__(self, conv_out_channels1, conv_out_channels2, conv_out_channels3, kernel_sizes, num_classes):
        ...
        self.convs1 = nn.ModuleList([nn.Conv1d(1, conv_out_channels1, kernel_sizes[0],stride=kernel_sizes[0]) for _ in range(126)])
        self.convs2 = nn.ModuleList([nn.Conv1d(1, conv_out_channels2, kernel_sizes[1], stride=kernel_sizes[1]) for _ in range(126)])
        self.convs3 = nn.ModuleList([nn.Conv1d(1, conv_out_channels3, kernel_sizes[2], stride=kernel_sizes[2]) for _ in range(126)])
        self.fc = nn.Linear(12096, self.num_classes)
...
if __name__ == "__main__":
    ...
    model_dir = f'/home/tauproj6/EEG_proj/patient_1_output_1.5s/annotate_model_19.07_seperate_conv1D/'
