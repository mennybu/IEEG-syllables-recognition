...
class CnnRnnClassifier(torch.nn.Module):
    ...
    def __init__(self,cnn_2_layer_dim, rnn_dim, KS, num_layers, dropout, n_classes, bidirectional, in_channels, keeptime=False,
                 token_input=None):
        super().__init__()

        self.preprocessing_conv = nn.Conv1d(in_channels=in_channels,
                                            out_channels=cnn_2_layer_dim,
                                            kernel_size=KS,
                                            stride=KS)

        ###########
        self.preprocessing_conv_layer2 = nn.Conv1d(in_channels=cnn_2_layer_dim,
    ...
    def forward(self, x):
        ...
        x = self.preprocessing_conv_layer2(x) # applying conv1D
        #         x = F.relu(x)
        x = self.dropout(x)
...
if __name__ == "__main__":
    ...
    data_dir = r'/home/tauproj6/EEG_proj/patient_1_output_3s_v2/annotate'

    label_to_idx = {'a': 0, 'e': 1, 'i': 2, 'o': 3, 'u': 4}

    # Define the directory where you want to save the model
    model_dir = f'/home/tauproj6/EEG_proj/patient_1_output_3s_v2/annotate_2layers_cnn_model_3layer_0.7do_01_07_lr0.0005/'
