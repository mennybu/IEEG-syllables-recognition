...
def save_parameters_to_file(params,  tw, filename):
    with open(filename, 'w') as file:
        ...
        file.write('\Model name : CnnRnnmodel with post cnn\n')
...
class CnnRnnClassifier(torch.nn.Module):
    def __init__(self, rnn_dim, KS, num_layers, dropout, n_classes, bidirectional, in_channels, keeptime=False, token_input=None):
        ...
        self.dense = nn.Linear(rnn_dim * mult * 10, n_classes)
    ...
    def forward(self, x):
        ...
        if not self.keeptime:
            ...
            x = output.contiguous().permute(1, 2, 0)
            x = self.postprocessing_conv(x)
            x = x.permute(0, 2, 1)
            t_samples = 10
            x = x[:, :-1:int(len(x[0])/t_samples), :]
...
if __name__ == "__main__":
    ...
    model_dir = f'/home/tauproj6/EEG_proj/patient_1_output_3s_v2/annotate_model_keeptime_3layer_0.7do_07_07_lr0.001_80cnn/'
